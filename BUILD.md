...
- [ ] 01 · Repo, `justfile`, `.gitignore`, `STATE.md`, Python package skeleton
- [ ] 02 · `000_init.sql` + `ledger migrate` with `user_version` + dump/restore round-trip test
- [ ] 03 · `ledger log` — writes an entry, prints the streak
...
- [ ] 06 · `001_claims.sql` + the sourcing trigger + its test
...
- [ ] 12 · `cargo tauri init`, window, WAL connection, `user_version` guard; pick vanilla vs Preact
- [ ] 13 · **Artboard 01** — every token as a CSS variable, fonts self-hosted
...
- [ ] 17 · **Artboard 02** — stat row, floor toggle
...
- [ ] 23 · **Artboard 05** — header, identity strip, summary strip
